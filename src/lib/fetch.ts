...
const Login = async (
    ...
    try {
        const response = await fetch(
            generateQueryUrl("auth/local", queryParams),
            {
                method: "POST",
...
/**
 * fetchGetAll   nigesh%2B13%40newagesmb.com nigesh%2B12%40newagesmb.com
 *
 * @param entity
 * @param queryParams
 * @param options
...
const DeleteByUid = async (
    ...
    try {
        const response = await fetch(
            generateQueryUrl(`${entity}/${uid}`, queryParams),
            {
                method: "DELETE",
                headers: await getHttpOption(options || {}),
                ...requestInit,
        ...
        // revalidatePath('/(protected)/users', 'page');
        return responseHandler(response);
    } catch (error) {
        return errorHandler(error);
    }
...
const Post = async (
    ...
    try {
        ...
        const t = await responseHandler(response);
        // ();
...
const Put = async (
    ...
    try {
        ...
        // (await getHttpOption(options || {}),"sssssssssssssssss");
